...
pub enum BrokerError {
    ...
    #[error("chat_completion is not implemented yet (S3 wires it)")]
    NotImplemented,
}
...
impl CodexBroker {
    ...
    /// S3 placeholder: real `chat_completion(req)` plumbing lives in
    /// `ask_runtime` (not yet implemented). S2 just returns an error
    /// so any accidental caller fails loud rather than silent.
    pub async fn chat_completion_placeholder(&self) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn chat_completion_placeholder_increments_counter_and_errors() {
        seed_test_key();
        let tmp = tempdir().unwrap();
        let broker = CodexBroker::new(tmp.path()).unwrap();

        let rt = tokio::runtime::Runtime::new().unwrap();
        let result = rt.block_on(async { broker.chat_completion_placeholder().await });
        assert!(matches!(result, Err(BrokerError::NotImplemented)));
        assert_eq!(broker.public_status().requests_today, 1);
    }
