...
pub(crate) fn install(router: Router<AppState>) -> Router<AppState> {
    router
        ...
        .route("/api/wiki/policies", get(get_policy_files_handler))
        ...
        .route(
            ...
            get(get_cross_domain_feedback_handler).post(post_cross_domain_feedback_handler),
        )
