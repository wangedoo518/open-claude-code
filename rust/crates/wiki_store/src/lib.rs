...
/// Process-global guard for `.clawwiki/vault-git-log.jsonl`. Git operations
/// can be triggered from several UI controls, so append audit writes are
/// serialized even though each line is independent.
static VAULT_GIT_AUDIT_GUARD: OnceLock<Mutex<()>> = OnceLock::new();

fn lock_vault_git_audit_writes() -> MutexGuard<'static, ()> {
    VAULT_GIT_AUDIT_GUARD
        .get_or_init(|| Mutex::new(()))
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
}
...
/// Machine-readable audit log for successful Buddy Vault Git mutations. This
/// lives under `.clawwiki/` but is intentionally ignored by Git so audit
/// bookkeeping never dirties a user's checkpoint state.
pub const VAULT_GIT_AUDIT_FILENAME: &str = "vault-git-log.jsonl";
...
const GITIGNORE_TEMPLATE: &str = r#"# Buddy Vault autogenerated .gitignore
...
# Local Git operation audit (never dirties checkpoint state)
.clawwiki/vault-git-log.jsonl
...
pub fn init_wiki(root: &Path) -> Result<()> {
    ...
    let root_claude_md_path = paths.root.join("CLAUDE.md");
    if !root_claude_md_path.exists() {
        fs::write(&root_claude_md_path, ROOT_CLAUDE_SHIM)
            .map_err(|e| WikiStoreError::io(root_claude_md_path.clone(), e))?;
    }

    let root_agents_md_path = paths.root.join("AGENTS.md");
    if !root_agents_md_path.exists() {
        fs::write(&root_agents_md_path, ROOT_AGENTS_SHIM)
            .map_err(|e| WikiStoreError::io(root_agents_md_path.clone(), e))?;
    }
    ...
    let purpose_lenses_path = paths.schema.join("purpose-lenses.yml");
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct VaultGitAuditLog {
    pub vault_path: String,
    pub entries: Vec<VaultGitAuditEntry>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct VaultGitAuditEntry {
...
fn vault_git_audit_path(paths: &WikiPaths) -> PathBuf {
    paths.meta.join(VAULT_GIT_AUDIT_FILENAME)
}

fn ensure_vault_git_audit_is_ignored(root: &Path) -> Result<()> {
    ...
    fs::create_dir_all(&info_dir).map_err(|e| WikiStoreError::io(info_dir.clone(), e))?;
    let exclude_path = info_dir.join("exclude");
    let ignore_line = format!("{META_DIR}/{VAULT_GIT_AUDIT_FILENAME}");
    let existing = fs::read_to_string(&exclude_path).unwrap_or_default();
    if existing
        .lines()
        .map(str::trim)
        .any(|line| line == ignore_line)
    {
        return Ok(());
    }
    ...
    next.push_str("# Buddy local Git operation audit\n");
    next.push_str(&ignore_line);
    next.push('\n');
    fs::write(&exclude_path, next).map_err(|e| WikiStoreError::io(exclude_path, e))?;
    Ok(())
}

fn append_vault_git_audit_entry(paths: &WikiPaths, entry: &VaultGitAuditEntry) -> Result<()> {
    let _guard = lock_vault_git_audit_writes();
    ensure_vault_git_audit_is_ignored(&paths.root)?;
    fs::create_dir_all(&paths.meta).map_err(|e| WikiStoreError::io(paths.meta.clone(), e))?;

    use std::io::Write;
    let path = vault_git_audit_path(paths);
    ...
    let line = serde_json::to_string(entry)
        .map_err(|e| WikiStoreError::Invalid(format!("git audit serialize error: {e}")))?;
...
fn record_vault_git_audit(
    ...
    if let Err(e) = append_vault_git_audit_entry(paths, &entry) {
        eprintln!("[vault-git-audit] soft-fail: {operation} audit append failed: {e}");
    }
}

pub fn vault_git_audit_log(paths: &WikiPaths, limit: usize) -> Result<VaultGitAuditLog> {
    let path = vault_git_audit_path(paths);
    if !path.is_file() {
        return Ok(VaultGitAuditLog {
    ...
    for line in content.lines().filter(|line| !line.trim().is_empty()) {
        match serde_json::from_str::<VaultGitAuditEntry>(line) {
            Ok(entry) => entries.push(entry),
            Err(e) => eprintln!("[vault-git-audit] skipped malformed line: {e}"),
        }
    }
    entries.reverse();
...
fn is_change_line_kind(kind: &str) -> bool {
...
fn change_block_bounds(hunk: &VaultGitDiffHunk, line_index: usize) -> Option<(usize, usize)> {
    ...
    while start > 0 && is_change_line_kind(&hunk.lines[start - 1].kind) {
        start -= 1;
    }
...
fn remove_exact_line(content: &str, line_number: u32, expected_text: &str) -> Result<String> {
    if line_number == 0 {
        return Err(WikiStoreError::Invalid(
            "line number must be one-based".to_string(),
        ));
    }
    ...
    let index = usize::try_from(line_number - 1).unwrap_or(usize::MAX);
    let Some(actual) = lines.get(index) else {
        return Err(WikiStoreError::Invalid(format!(
            "line is no longer present in Buddy Vault: {line_number}"
        )));
    };
    let actual_text = actual.trim_end_matches('\n').trim_end_matches('\r');
...
pub fn vault_git_set_remote(
    ...
    record_vault_git_audit(
        paths,
        "remote",
        &format!("{remote} -> {}", redact_remote_url(url)),
        None,
        None,
        None,
        None,
...
pub fn vault_git_discard_path(paths: &WikiPaths, path: &str) -> Result<VaultGitDiscardResult> {
    ...
    let status = vault_git_status(paths)?;
    let summary = format!("Discarded {mode} changes for {path}");
    record_vault_git_audit(
        paths,
        "discard-path",
        &summary,
...
pub fn vault_git_discard_hunk(
    ...
    let status = vault_git_status(paths)?;
    let summary = format!("Discarded hunk {} for {path}", hunk_index + 1);
    record_vault_git_audit(
        paths,
        "discard-hunk",
        &summary,
...
pub fn vault_git_discard_added_line(
    paths: &WikiPaths,
    path: &str,
    hunk_index: usize,
    line_index: usize,
    ...
    if change.xy == "??" || change.unstaged == " " {
        return Err(WikiStoreError::Invalid(
            "line discard only supports tracked unstaged changes".to_string(),
        ));
    }
    ...
    if line.kind != "add" {
        return Err(WikiStoreError::Invalid(
            "line discard only supports added lines".to_string(),
        ));
    }
    if !is_standalone_added_line(hunk, line_index) {
        return Err(WikiStoreError::Invalid(
            "line discard only supports standalone added lines; discard the hunk to restore replacements"
                .to_string(),
        ));
    }
    ...
    let Some(line_number) = line.new_line else {
        return Err(WikiStoreError::Invalid(
            "added line is missing a working-tree line number".to_string(),
        ));
    };

    let target = paths.root.join(&path);
    let content = fs::read_to_string(&target).map_err(|e| WikiStoreError::io(target.clone(), e))?;
    let next_content = remove_exact_line(&content, line_number, &line.text)?;
    fs::write(&target, next_content).map_err(|e| WikiStoreError::io(target.clone(), e))?;

    let status = vault_git_status(paths)?;
    let summary = format!(
        "Discarded added line {} in hunk {} for {path}",
        line_index + 1,
        hunk_index + 1
    );
    record_vault_git_audit(
...
pub struct WikiFrontmatter {
    ...
    pub summary: String,
    /// Purpose Lens values used by Buddy to route the page into express
    /// workflows. Defaults to "learning" for maintainer-created pages
...
pub struct WikiPageSummary {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub expressed_in: Vec<String>,
    /// Source references from frontmatter `source_refs`, e.g. `raw:00001`
...
/// Read the complete markdown file for a wiki concept page, including
/// YAML frontmatter. Used by the Buddy wiki editor so humans can edit
...
/// Overwrite a wiki concept page with complete markdown content,
/// including frontmatter. This is the human edit path; it deliberately
/// preserves every user-supplied frontmatter field instead of
/// rebuilding YAML from a narrow Rust struct.
pub fn overwrite_wiki_page_content(
    paths: &WikiPaths,
    slug: &str,
    content: &str,
) -> Result<PathBuf> {
...
fn validate_wiki_page_markdown_content(content: &str) -> Result<()> {
    if content.trim().is_empty() {
        return Err(WikiStoreError::Invalid(
            "wiki page content must not be empty".to_string(),
        ));
    }
    let mut lines = content.lines();
    if lines.next() != Some("---") {
    ...
    for line in lines.by_ref() {
        ...
        if collecting_purpose {
            if let Some(rest) = trimmed.strip_prefix("- ") {
                validate_purpose_lens_value(rest)?;
                continue;
            }
    ...
    if !has_type || !has_status || !has_title {
        return Err(WikiStoreError::Invalid(
            "wiki page frontmatter must include non-empty type, status, and title".to_string(),
        ));
    }
    Ok(())
}
...
fn validate_purpose_lens_value(raw: &str) -> Result<()> {
    let lens = raw
        .trim()
        .trim_matches('"')
        .trim_matches('\'')
    ...
    if lens.is_empty() {
        return Err(WikiStoreError::Invalid(
            "purpose lens must not be empty".to_string(),
        ));
    }
    let valid = lens
        .chars()
...
/// Pull `title`, `summary`, lineage refs, and timestamps out of the
...
fn parse_wiki_frontmatter_fields(
    ...
    let mut collecting_source_refs = false;
    for line in content.lines() {
        if line == "---" {
            if in_frontmatter {
                break;
        ...
        if collecting_purpose {
            if let Some(rest) = trimmed.strip_prefix("- ") {
                push_purpose_value(&mut purpose, rest);
                continue;
            }
            if !line.starts_with(' ') && !line.starts_with('\t') {
                collecting_purpose = false;
        ...
        if collecting_source_refs {
            if let Some(rest) = trimmed.strip_prefix("- ") {
                push_expressed_ref_value(&mut source_refs, rest);
                continue;
            }
            if !line.starts_with(' ') && !line.starts_with('\t') {
                collecting_source_refs = false;
            }
        }
...
fn parse_inline_purpose_values(out: &mut Vec<String>, raw: &str) {
    ...
    for part in trimmed.split(',') {
        push_purpose_value(out, part);
    }
}

fn push_purpose_value(out: &mut Vec<String>, raw: &str) {
    let lens = raw
        .trim()
        .trim_matches('"')
        .trim_matches('\'')
        .to_ascii_lowercase();
    if lens.is_empty() || out.iter().any(|existing| existing == &lens) {
        return;
    }
    out.push(lens);
}
...
mod tests {
    ...
    #[test]
    fn init_wiki_seeds_root_agent_shims() {
        let tmp = tempdir().unwrap();
        init_wiki(tmp.path()).unwrap();
        let root_agents = tmp.path().join("AGENTS.md");
    ...
    #[test]
    fn overwrite_wiki_page_content_preserves_full_frontmatter() {
        let tmp = tempdir().unwrap();
        init_wiki(tmp.path()).unwrap();
        let paths = WikiPaths::resolve(tmp.path());
    ...
    #[test]
    fn overwrite_wiki_page_content_rejects_invalid_frontmatter() {
        let tmp = tempdir().unwrap();
        init_wiki(tmp.path()).unwrap();
        let paths = WikiPaths::resolve(tmp.path());
        write_wiki_page(&paths, "editable", "Editable", "summary", "Body.", None).unwrap();

        let err = overwrite_wiki_page_content(&paths, "editable", "# No frontmatter").unwrap_err();
        assert!(matches!(err, WikiStoreError::Invalid(_)));

        let err = overwrite_wiki_page_content(
            &paths,
            "editable",
            "---\ntype: concept\nstatus: active\npurpose:\n  - Research Space\n---\nBody",
        )
    ...
    fn init_wiki_seeds_gitignore_with_canonical_entries() {
        ...
        assert!(
            content.contains(".clawwiki/vault-git-log.jsonl"),
            "vault git audit log not excluded"
        );
    ...
    fn vault_git_discard_hunk_restores_only_selected_hunk() {
        ...
        let audit = vault_git_audit_log(&paths, 10).unwrap();
        assert!(audit.entries.iter().any(|entry| {
            entry.operation == "discard-hunk"
                && entry.path.as_deref() == Some("wiki/concepts/hunked.md")
                && entry.hunk_index == Some(0)
        }));
    }
    ...
    fn vault_git_discard_added_line_removes_only_selected_line() {
        ...
        let page = paths.wiki.join("concepts").join("line-discard.md");
        ...
        let section = diff
            .sections
            .iter()
            .find(|section| section.path == "wiki/concepts/line-discard.md")
            .expect("line discard diff section");
        let (hunk_index, hunk, line_index, line) = section
            ...
            .expect("inserted line metadata");

        let discarded = vault_git_discard_added_line(
            &paths,
            "wiki/concepts/line-discard.md",
            hunk_index,
            line_index,
            Some(&hunk.header),
            Some(&line.text),
        ...
        let audit = vault_git_audit_log(&paths, 10).unwrap();
        assert!(audit.entries.iter().any(|entry| {
            entry.operation == "discard-line"
                && entry.path.as_deref() == Some("wiki/concepts/line-discard.md")
    ...
    #[test]
    fn vault_git_discard_added_line_rejects_replacement_edits() {
        let Some((_tmp, paths)) = git_test_paths() else {
            return;
        };
        let page = paths.wiki.join("concepts").join("line-replace.md");
        fs::write(&page, hunk_test_body("line 02", "line 70")).unwrap();
        vault_git_commit(&paths, "Initial Buddy Vault checkpoint").unwrap();
        ...
        let section = diff
            .sections
            .iter()
            .find(|section| section.path == "wiki/concepts/line-replace.md")
            .expect("replacement diff section");
        let (line_index, line) = section.hunks[0]
            .lines
            .iter()
            .enumerate()
            .find(|(_, line)| line.kind == "add" && line.text == "line 02 changed")
            .expect("replacement added line");

        let err = vault_git_discard_added_line(
            &paths,
            "wiki/concepts/line-replace.md",
            0,
            line_index,
