...
/**
 * Safe-default useSidebar — historically threw when used outside a
 * SidebarProvider, which surfaces real bugs cleanly in production but
 * floods the dev console with React-19's transient-render artifacts
 * (StrictMode-style double invocations briefly run children before the
 * Provider's effect commits, even when the Provider is correctly in
 * the tree). We keep the dev warning so contributors notice misuse,
 * but degrade gracefully instead of crashing the render — ErrorBoundary
 * doesn't have to recover, and the component just renders default
 * "expanded, no-op" state until the real context arrives next pass.
 */
