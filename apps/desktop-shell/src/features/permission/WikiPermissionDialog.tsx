// S0.3 extraction target: the permission-gate dialog (CCD soul ③).
//
// Original: features/session-workbench/PermissionDialog.tsx.
// Body is a verbatim port; only the component name and the types
// import path change. The dialog will gate every `write_page` /
// `patch_page` / `deprecate_page` / `mark_conflict` the wiki_maintainer
// emits once S4 is wired, per ClawWiki canonical §6.1.
...
interface WikiPermissionDialogProps {
  request: PermissionRequest;
  onDecision: (action: PermissionAction) => void;
}

export function WikiPermissionDialog({
  request,
  onDecision,
}: WikiPermissionDialogProps) {
